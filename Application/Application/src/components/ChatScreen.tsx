...
const ChatScreen = () => {
  ...
  const [messages, setMessages] = useState<
    { role: string; text: string; timeTaken?: number; sources?: string[] }[]
  ...
  const sendQuestionToAPI = async (question: string) => {
    ...
    try {
      ...
      return {
        answer: "Sorry, there was an error getting your answer.",
        timeTaken: null,
        sources: [],
      };
    } finally {
  ...
  const handleSend = async () => {
    ...
    const { answer, timeTaken, sources } = await sendQuestionToAPI(question);
    setIsTyping(false);
    ...
    typeOutAnswer(answer, timeTaken, sources, () => {});
  };
                ...
                {msg.role === "bot" && (
                  <div className="mt-1 text-xs text-gray-400">
                    {msg.timeTaken !== undefined && (
                      <div>Response time: {msg.timeTaken}s</div>
                    )}
